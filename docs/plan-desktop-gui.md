...
価値ネット `value_mid_v3_10x128_t40` の held-out AUC（iter1400 以降の世代。2026-09-08）:
...
全体 0.70。`docs/plan-search-at-playtime.md` の 0.727〜0.730 は gen≤578 で切った held-out の値で、
現行世代で測ると旧ネットでも 0.686 に下がる。ネットが古くなったのではなく、局面の分布が 0 に寄って
判別が本当に難しくなった（41 手目 |cp|>1000 の割合が iter171 の 37.8% から iter1177 の 27.1% へ）。
**0.69〜0.70 がいまの現実**で、序盤は弱い。**弱いことを画面で隠さない**（信頼帯を出す。後述）。
...
**段差は測った（2026-09-08、M0）。** iter1177 の 5,569 局で V(s_40) − p(cp_41) は
t=40 を教師に含む v3 で平均 −0.007（標準偏差 0.26）。**定数補正は要らず、同じ目盛りで繋ぐ。**
t=40 を含まない v2 では平均 −0.151 だった。詳細は `docs/usi-fuseki-extension.md`。
...
| 段 | 成果物 | 測ること（合否） |
|---|---|---|
| M0 設計（済） | 4.2 の USI 拡張仕様を `docs/` に固定。41 手目の段差の較正（GPU のみ） | 平均差 −0.7pt（v3）。定数補正は入れない |
...
- **41 手目の段差**。測った。t=40 を教師に入れた v3 で消えた（平均 −0.7pt）。補正も印も要らない。
