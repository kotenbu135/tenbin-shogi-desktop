...
- 天秤将棋には禁じ手「二飛香」がある（2026-09-13 決定。3〜40 手目、自陣の同じ筋に自分の飛・香を合わせて 2 枚以上打てない）。
  GUI は棋譜ごとにルールの版（`1`=二飛香の前、`2`=いま）を持ち、内蔵の評価には `setoption name Fuseki_Rules value 1|2` の形で渡す
  （内蔵の評価だけが持つ口。`Fuseki_Mode=fuseki` のときは版を見ない）。**外のエンジンには送らない**ので、外のエンジンが
  二飛香を知らなければ反則手を返しうる。返した手は GUI が wasm の合法手と照合して弾く
