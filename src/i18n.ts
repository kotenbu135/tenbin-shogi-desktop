...
const DICT = {
  ...
  over_sennichite: { ja: '千日手', en: 'Fourfold repetition (sennichite)' },
  over_perpetual_check: { ja: '{side}の連続王手の千日手', en: 'Perpetual check by {side}' },
  over_declaration: { ja: '{side}の入玉宣言', en: '{side} declares an entering-king win' },
  over_illegal_declaration: {
    ja: '{side}の入玉宣言が条件を満たさない',
    en: '{side} declared an entering-king win without meeting the conditions',
  },
  over_max_moves: { ja: '本将棋が{n}手に達した', en: 'The shogi phase reached {n} moves' },
  over_other: { ja: '終局', en: 'Game over' },
  err_game_over: { ja: '対局は終わっている', en: 'The game is already over' },
  err_declare_not_normal: {
    ja: '入玉宣言は本将棋（41手目以降）でだけできる',
    en: 'An entering-king declaration is only possible in the shogi phase (from move 41)',
  },
  ...
  tb_declare_title: {
    ja: '入玉宣言（27点法）。条件を欠いて宣言すると負け',
    en: 'Declare an entering-king win (27-point rule). Declaring without meeting the conditions loses',
  },
  ...
  confirm_declare: {
    ja: '{side}が入玉宣言しますか？\n玉が敵陣三段目以内・王手されていない・敵陣の駒（玉を除く）が10枚以上・持ち駒と敵陣の駒が先手28点／後手27点以上（大駒5点、他1点）。1つでも欠けると負けです',
    en: 'Declare an entering-king win as {side}?\nKing inside the enemy camp, not in check, at least 10 other pieces in the enemy camp, and at least 28 points for Sente / 27 for Gote from those pieces and the hand (rook/bishop 5, others 1). Missing any condition loses.',
  },
  alert_declare_not_your_turn: {
    ja: 'いまは相手の手番です。入玉宣言は自分の手番にだけできます',
    en: 'It is the opponent’s turn. You can only declare on your own turn',
  },
  ...
  cuda_switch_need: {
    ja: 'CUDA 12 と cuDNN 9 は NVIDIA のもので、このアプリにもエンジンにも入っていません。インストーラはこの画面から取ってこられるので、インストールだけご自身で行ってください（合わせて数 GB）。',
    en: 'CUDA 12 and cuDNN 9 come from NVIDIA and are not included in this app or the engine. This window can download the installers; you only run them yourself (several GB in total).',
  },
  cuda_fallback_title: { ja: 'CUDA に切り替えられませんでした', en: 'Could not switch to CUDA' },
  cuda_fallback_lead: {
    ja: '「{name}」は CUDA を試しましたが、足りない DLL があって使えず、{provider} で読んでいます（止まってはいませんが、CUDA 版の DLL では DirectML に戻れないので CPU だと DirectML 版より遅くなります）。ONNX Runtime のエラー文（足りない DLL の名前が無いときは、USI ログの標準エラーの行の <code>which depends on "…dll" which is missing</code> に出ています）:',
    en: '“{name}” tried CUDA but a library is missing, so it is running on {provider} instead (it has not stopped, but the CUDA build cannot fall back to DirectML, so on the CPU it is slower than the DirectML build). The ONNX Runtime error (if it does not name the missing DLL, look for <code>which depends on "…dll" which is missing</code> in the stderr lines of the USI log):',
  },
  cuda_fallback_fix: { ja: '足りないものを下で揃えてから、エンジンを立て直してください。', en: 'Get the missing pieces below, then restart the engine.' },
  cuda_checking: { ja: '入っているものを調べています…', en: 'Checking what is installed…' },
  cuda_check_cuda: { ja: 'CUDA 12', en: 'CUDA 12' },
  cuda_check_cudnn: { ja: 'cuDNN 9（CUDA 12 用）', en: 'cuDNN 9 (for CUDA 12)' },
  cuda_check_ort: { ja: 'CUDA 版の ONNX Runtime（エンジンのフォルダ）', en: 'CUDA build of ONNX Runtime (engine folder)' },
  cuda_have: { ja: 'あり', en: 'found' },
  cuda_have_not: { ja: 'なし', en: 'missing' },
  cuda_fetch: { ja: '足りないインストーラをダウンロード（{size}）', en: 'Download the missing installers ({size})' },
  cuda_fetch_hint: {
    ja: 'NVIDIA の配布元から「ダウンロード」フォルダに取ってきます。インストールはご自身で行ってください。',
    en: 'Downloads them from NVIDIA into your Downloads folder. You run the installation yourself.',
  },
  cuda_fetched: {
    ja: 'ダウンロードしました（フォルダを開きました）。次の順にインストールしてから「確かめ直す」を押してください。アプリの開き直しは要りません。',
    en: 'Downloaded (the folder is open). Install them in this order, then press “Check again”. You do not need to reopen this app.',
  },
  cuda_install_cuda: {
    ja: '<code>{file}</code>（CUDA 12）を開く →「カスタム（詳細）」を選び、「CUDA」だけに印を残して進める（画面のドライバーは入れ替えない。インストール中に数 GB 取ってきます）',
    en: 'Open <code>{file}</code> (CUDA 12) → choose “Custom (Advanced)”, keep only “CUDA” ticked and continue (this leaves your display driver alone; it downloads a few GB while installing)',
  },
  cuda_install_cudnn: { ja: '<code>{file}</code>（cuDNN 9）を開く → そのまま進める', en: 'Open <code>{file}</code> (cuDNN 9) → accept the defaults' },
  cuda_recheck: { ja: '確かめ直す', en: 'Check again' },
  cuda_swap: { ja: 'CUDA 版に切り替える（約 380MB）', en: 'Switch to the CUDA build (about 380 MB)' },
  cuda_swap_hint: {
    ja: 'ONNX Runtime 1.30.0 の CUDA 版を取ってきて、エンジンのフォルダの DLL を差し替えます。元の DLL は残すので DirectML 版に戻せます。動いているエンジンは止めます。',
    en: 'Downloads the CUDA build of ONNX Runtime 1.30.0 and replaces the DLLs in the engine folder. The original DLLs are kept, so you can go back to DirectML. Running engines are stopped.',
  },
  cuda_swapped: {
    ja: '「{name}」を CUDA 版に切り替えました。次にエンジンが立ち上がると CUDA で読みます（USI ログに「推論: CUDA」と出ます）',
    en: 'Switched “{name}” to the CUDA build. It will use CUDA the next time the engine starts (the USI log says “Inference: CUDA”)',
  },
  cuda_ready: {
    ja: '揃っています。エンジンを立て直すと CUDA で読みます（USI ログに「推論: CUDA」と出ます）。',
    en: 'Everything is in place. Restart the engine and it will use CUDA (the USI log says “Inference: CUDA”).',
  },
  cuda_restore: { ja: 'DirectML 版に戻す', en: 'Go back to DirectML' },
  cuda_restored: { ja: '「{name}」を DirectML 版に戻しました', en: 'Put “{name}” back on DirectML' },
  cuda_failed: { ja: 'できませんでした: {msg}', en: 'Failed: {msg}' },
  cuda_pages: { ja: 'うまくいかないときは NVIDIA の配布ページから:', en: 'If this does not work, use NVIDIA’s download pages:' },
  cuda_manual: { ja: '手で入れるとき', en: 'Doing it by hand' },
  cuda_step1: {
    ja: 'ONNX Runtime 1.30.0 の配布ページから <code>onnxruntime-win-x64-gpu_cuda12-1.30.0.zip</code> を取り、その <code>lib/</code> の <code>onnxruntime.dll</code>・<code>onnxruntime_providers_cuda.dll</code>・<code>onnxruntime_providers_shared.dll</code> を、エンジンのフォルダ（実行ファイルのあるフォルダ。下の「エンジンのフォルダを開く」で開く）に写し、同名のファイルに上書きする。書庫を展開しただけでは読まれない（<code>DirectML.dll</code> は残してよい。元の <code>onnxruntime.dll</code>・<code>onnxruntime_providers_shared.dll</code> を別の名前で残しておけば DirectML 版に戻せる）',
    en: 'From the ONNX Runtime 1.30.0 release page, get <code>onnxruntime-win-x64-gpu_cuda12-1.30.0.zip</code> and copy <code>onnxruntime.dll</code>, <code>onnxruntime_providers_cuda.dll</code> and <code>onnxruntime_providers_shared.dll</code> from its <code>lib/</code> into the engine folder (the folder of the executable; “Open the engine folder” below opens it), overwriting the files of the same name. Just extracting the zip elsewhere does nothing (<code>DirectML.dll</code> can stay; keep the original <code>onnxruntime.dll</code> and <code>onnxruntime_providers_shared.dll</code> under another name to go back to DirectML)',
  },
  cuda_step2: {
    ja: 'NVIDIA から CUDA Toolkit 12.x と cuDNN 9.x を入れる。標準の場所（<code>C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v12.x</code> と <code>C:\\Program Files\\NVIDIA\\CUDNN\\v9.x\\bin\\12.x\\x64</code>）ならこのアプリが見つけてエンジンに渡すので、<code>Path</code> は変えなくてよい。別の場所に入れたときは、<code>cudart64_12.dll</code>・<code>cublas64_12.dll</code>・<code>cublasLt64_12.dll</code>・<code>cufft64_11.dll</code> と <code>cudnn*64_9.dll</code> 一式のフォルダを <code>Path</code> に足すか、エンジンのフォルダに置く（cuDNN の <code>13.x</code> は CUDA 13 用で読めない）',
    en: 'Install CUDA Toolkit 12.x and cuDNN 9.x from NVIDIA. In the standard locations (<code>C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v12.x</code> and <code>C:\\Program Files\\NVIDIA\\CUDNN\\v9.x\\bin\\12.x\\x64</code>) this app finds them and hands them to the engine, so you do not need to change <code>Path</code>. If you installed them elsewhere, add the folders holding <code>cudart64_12.dll</code>, <code>cublas64_12.dll</code>, <code>cublasLt64_12.dll</code>, <code>cufft64_11.dll</code> and all of <code>cudnn*64_9.dll</code> to <code>Path</code>, or put the DLLs in the engine folder (cuDNN’s <code>13.x</code> folder is for CUDA 13 and will not load)',
  },
  cuda_step3: {
    ja: 'エンジンを立て直し（検討を止めて始め直すなど。<code>Path</code> を変えたときは、このアプリを閉じて開き直す。環境変数は起動したときにしか読まれない）、USI ログに「推論: CUDA」と出れば完了',
    en: 'Restart the engine (for example, stop and restart the analysis; if you changed <code>Path</code>, close and reopen this app, since environment variables are only read at launch); when the USI log says “Inference: CUDA”, you are done',
  },
  cuda_tested: {
